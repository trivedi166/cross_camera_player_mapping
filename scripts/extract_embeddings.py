...
for frame_file in frame_files:
    ...
    for box in boxes:
        ...
        try:
            ...
            bboxes.append([int(x1), int(y1), int(x2), int(y2)])  # ← save box
    ...
    # Save bounding boxes
    box_path = out_path.replace(".npy", "_boxes.npy")
    np.save(box_path, np.array(bboxes))
    print(f"[INFO] Saved {len(embeddings)} embeddings -> {out_path}")
    print(f"[INFO] Saved {len(bboxes)} boxes -> {box_path}")
